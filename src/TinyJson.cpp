...
void *TinyJson::InternalStack::push(size_t len) {
  ...
  if (top + len > size) {
    ...
    ptr = static_cast<char *>(
        realloc(ptr, size)); // realloc保证旧内存复制到新地址
  }
  void *ret = ptr + top;
  top += len;
  return ret;
...
TinyJson::ParseResult TinyJson::parse_value(Node *v) {
  ...
  if (visitedPos.size() == 0) {
    return TinyJson::ParseResult::EXPECT_VALUE; // 比如 key:然后value啥也没有
  }
  switch (visitedPos[0]) {
  case 'n':
  ...
  default:
    ...
    // default: return TinyJson::ParseResult::INVALID_VALUE; //
    // 比如除了null/true/false以外,非字符串,key:test => test就是无效的
  }
}

void TinyJson::parse_whitespace() {
  std::string_view &sv = visitedPos; // 需要修改内部状态
  //sv.remove_prefix(std::min(sv.find_first_not_of(" \t\n\r"), sv.size()));
  size_t len = sv.size(), i = 0;
  while (i < len) {
    char c = sv[i];
...
TinyJson::ParseResult TinyJson::parse_number(TinyJson::Node *v) {
  std::string_view &sv = visitedPos; // 暂时不修改原来的string_view
...
TinyJson::ParseResult TinyJson::parse_string_raw(char *&str, std::size_t &len) {
  std::string_view &sv = visitedPos;
  assert(sv.front() == '\"'); // 第一个字符一定是'\"'
  size_t head = inStack->top; // 记录栈基位置
  ...
  for (++i; i < svLen;) { // 跳过第一个"
    ...
    switch (ch) {
    case '\"':
      // string结束m, 存在空字符串的可能, 在线平台上允许空字符串的存在
      // https://www.sojson.com/simple_json.html
      strLen = inStack->top - head; // strLen可能为0
      str = static_cast<char *>(inStack->pop(strLen));
      len = strLen;
      sv.remove_prefix(i); // 成功解析string, 否则sv的状态不变
      return ParseResult::OK;
    case '\\': // 处理转义字符
      if (i == svLen) {
        inStack->top = head;
        return ParseResult::INVALID_CHARACTER_ESCAPE; /* 如果到达字符尾端结束,
                                                         否则至少还存在一位 */
      }
      switch (sv[i++]) {
      case '\"':
...
void TinyJson::encode_utf8(unsigned u) {
  if (u <= 0x7F) {
    *static_cast<char *>(inStack->push(1)) = static_cast<char>(u & 0xFF);
  } else if (u <= 0x7FF) {
    char *start = static_cast<char *>(inStack->push(2));
    *start = static_cast<char>(0xC0 | ((u >> 6) & 0xFF));
    *(start + 1) = 0x80 | (u & 0x3F);
  } else if (u <= 0xFFFF) {
    char *start = static_cast<char *>(inStack->push(3));
    *start = static_cast<char>(0xE0 | ((u >> 12) & 0xFF));
    *(start + 1) = static_cast<char>(0x80 | ((u >> 6) & 0x3F));
    *(start + 2) = static_cast<char>(0x80 | (u & 0x3F));
  } else {
    char *start = static_cast<char *>(inStack->push(4));
    *start = static_cast<char>(0xF0 | ((u >> 18) & 0xFF));
    *(start + 1) = static_cast<char>(0x80 | ((u >> 12) & 0x3F));
    *(start + 2) = static_cast<char>(0x80 | ((u >> 6) & 0x3F));
    *(start + 3) = static_cast<char>(0x80 | (u & 0x3F));
  }
}
...
TinyJson::ParseResult TinyJson::parse_object(Node *v) {
  ...
  // 释放暂时存储在栈中对象的内存
  for (size_t i = 0; i < objSizse; ++i) {
    // 这里同样使用和之前array一样的笨方法...
    // 如果inStack是对齐的, 那么可以直接在上边原地析构
    // 其实是ub行为, 因为字节可能不对齐
    KVnode *kv = static_cast<KVnode *>(inStack->pop(sizeof(KVnode)));
    ::free(kv->str);
    JsonValue val(kv->v);
  }
  return ret;
